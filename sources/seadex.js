class SeadexExtension {
  ...
  async single({ anilistId, titles, episodeCount }) {
    ...
    const response = await fetch(`${this.url}?page=1&perPage=1&filter=alID%3D%22${anilistId}%22&skipTotal=1&expand=trs`);
    const { items } = await response.json();
    if (!items[0]?.expand?.trs?.length) return [];
    const { trs } = items[0].expand;
    return trs.filter(({ infoHash, files }) => infoHash !== "<redacted>" && (!episodeCount || episodeCount === 1 || files.length === 1)).map((torrent) => ({
      ...
      size: torrent.files.reduce((sum, file) => sum + file.length, 0),
...
export default new SeadexExtension();
